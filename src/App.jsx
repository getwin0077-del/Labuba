...
// --- Types
/** @typedef {{ id: string; name: string; icon: any; dmg: [number, number]; particles?: string[]; status?: "burn"|"freeze"|"shock"|"paint"|"slime"|"wind"|"wet"|"grow"|"shrink"|"ghost"; knock?: number; cooldown?: number; unlockAt?: number; comboBonus?: number; desc?: string; rarity?: 'common'|'rare'|'epic'|'legend'; }} Tool */

// --- Tool Catalog
/** @type {Tool[]} */
const TOOLS_BASE = [
  { id: "hammer", name: "Молот", icon: Hammer, dmg: [8, 14], knock: 18, desc: "Классический удар.", rarity:'common' },
  { id: "saw", name: "Пила", icon: Saw, dmg: [3, 6], knock: 4, status: "shock", comboBonus: 1, desc: "Ж-ж-ж-ж...", rarity:'common' },
  { id: "laser", name: "Лазер", icon: Wand2, dmg: [6, 10], particles: ["✦","✧","★"], status: "shock", desc: "Будущее здесь.", rarity:'rare', unlockAt: 60 },
  { id: "fire", name: "Огонь", icon: Flame, dmg: [5, 8], particles: ["🔥"], status: "burn", desc: "Поджарим!", rarity:'common' },
  { id: "ice", name: "Лёд", icon: Snowflake, dmg: [4, 7], particles: ["❄️"], status: "freeze", desc: "Заморозка.", rarity:'common' },
  { id: "water", name: "Вода", icon: Droplets, dmg: [2, 5], particles: ["💧"], status: "wet", desc: "Мокро и смешно.", rarity:'common' },
  { id: "paint", name: "Краска", icon: Paintbrush2, dmg: [2, 4], particles: ["🎨","🟡","🟣","🟢"], status: "paint", desc: "Артист!", rarity:'common' },
  { id: "wind", name: "Ветер", icon: Wind, dmg: [1, 3], knock: 30, status: "wind", desc: "Сдувает.", rarity:'common' },
  { id: "tnt", name: "Бум!", icon: Bomb, dmg: [10, 25], particles: ["💥","✨"], knock: 44, cooldown: 800, desc: "Конфетти-взрыв.", rarity:'epic', unlockAt: 120 },
  { id: "zap", name: "Молния", icon: Zap, dmg: [6, 12], particles: ["⚡"], status: "shock", desc: "Энергия!", rarity:'rare', unlockAt: 80 },
  { id: "anvil", name: "Наковальня", icon: Anvil, dmg: [9, 18], knock: 50, desc: "Тяжесть комедии.", rarity:'rare', unlockAt: 90 },
  { id: "feather", name: "Пёрышко", icon: Feather, dmg: [1, 1], particles: ["🪶"], desc: "Щекотка.", rarity:'common' },
  { id: "sun", name: "Солнце", icon: Sun, dmg: [3, 6], particles: ["☀️","✨"], desc: "Лучисто.", rarity:'common' },
  { id: "rocket", name: "Ракета", icon: Rocket, dmg: [8, 15], particles: ["🚀","✨"], knock: 36, cooldown: 600, desc: "Свист и бац!", rarity:'epic', unlockAt: 150 },
  { id: "drill", name: "Дрель", icon: Drill, dmg: [4, 7], particles: ["🌀"], status: "shock", desc: "Ж-ж-ж!", rarity:'common' },
  { id: "scissors", name: "Ножницы", icon: Scissors, dmg: [3, 6], desc: "Подравняем мех.", rarity:'common' },
  { id: "magnet", name: "Магнит", icon: Magnet, dmg: [2, 4], desc: "Притягательная беда.", rarity:'common' },
  { id: "slime", name: "Слизь", icon: Bug, dmg: [2, 3], particles: ["🟩","🧪"], status: "slime", desc: "Липко.", rarity:'common' },
  { id: "flower", name: "Цветы", icon: Flower2, dmg: [1, 2], particles: ["🌸","🌼"], desc: "Чих!", rarity:'common' },
  { id: "storm", name: "Гроза", icon: CloudLightning, dmg: [7, 12], particles: ["⛈️"], status: "shock", desc: "Громыхаем.", rarity:'rare', unlockAt: 70 },
  { id: "rain", name: "Ливень", icon: CloudRain, dmg: [2, 4], particles: ["🌧️"], status: "wet", desc: "Мокренько.", rarity:'common' },
  { id: "snow", name: "Снег", icon: CloudSnow, dmg: [2, 5], particles: ["❄️"], status: "freeze", desc: "Зима.", rarity:'common' },
  { id: "ghost", name: "Призрак", icon: Ghost, dmg: [5, 9], status: "ghost", desc: "Бу!", rarity:'rare', unlockAt: 110 },
  { id: "shield", name: "Щит Лабубы", icon: ShieldCheck, dmg: [0, 0], desc: "Временная защита.", rarity:'rare', unlockAt: 50 },
  { id: "heal", name: "Супчик (+)", icon: Soup, dmg: [-15, -10], desc: "Лечит за 20 монет.", rarity:'common' },
  { id: "coin", name: "Монеты", icon: Coins, dmg: [0, 0], desc: "+10 монет.", rarity:'common' },
  { id: "gift", name: "Подарок", icon: Gift, dmg: [0, 0], desc: "Сюрприз-бонус.", rarity:'epic', unlockAt: 140 },
  { id: "reset", name: "Сброс", icon: RefreshCw, dmg: [0, 0], desc: "Перезапуск боя.", rarity:'common' },
  // легендарки
  { id: "starfall", name: "Звездопад", icon: Star, dmg: [14,22], particles:["✨","⭐","🌟"], knock:40, cooldown:900, desc:"Красивая буря частиц.", rarity:'legend', unlockAt: 220 },
  { id: "dice", name: "Куб удачи", icon: Dice5, dmg: [0,30], particles:["🎲","✨"], knock:20, cooldown:900, desc:"Может ничего, а может — вау!", rarity:'legend', unlockAt: 250 },
];

const ACHIEVEMENTS = [
  { id: "firstHit", title: "Первый шлеп", cond: (s) => s.hits >= 1, reward: 5, icon: Trophy },
  { id: "tenHit", title: "Десяток!", cond: (s) => s.hits >= 10, reward: 20, icon: Trophy },
  { id: "hundredHit", title: "Соточка", cond: (s) => s.hits >= 100, reward: 200, icon: Trophy },
  { id: "combo25", title: "Комбо 25", cond: (s) => s.combo >= 25, reward: 30, icon: Loader2 },
  { id: "burn", title: "Поджарили", cond: (s) => s.statusSeen.burn, reward: 10, icon: Flame },
  { id: "freeze", title: "Остудили", cond: (s) => s.statusSeen.freeze, reward: 10, icon: Snowflake },
  { id: "shock", title: "Электрошок!", cond: (s) => s.statusSeen.shock, reward: 10, icon: Zap },
  { id: "paint", title: "Артист", cond: (s) => s.statusSeen.paint, reward: 10, icon: Paintbrush2 },
  { id: "slime", title: "Липучка", cond: (s) => s.statusSeen.slime, reward: 10, icon: Bug },
  { id: "bossWin", title: "Босс повержен", cond: (s) => s.bossKills >= 1, reward: 100, icon: Skull },
  { id: "legend", title: "Легенда!", cond: (s)=> s.legendUsed, reward: 150, icon: Star },
];
...
export default function App(){
  ...
  const toolButtons = useMemo(()=> tools.map(t=>{
    const Icon = t.icon; const disabled = onCd(t.id); const locked = !owned[t.id];
    return (
    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-amber-50 to-sky-50 text-zinc-900 p-4">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-4">
        ...
        <div className="rounded-2xl bg-white/70 backdrop-blur p-4 shadow-lg border">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <button onClick={()=>setShowShop(true)} className="px-3 py-2 rounded-lg bg-white/80 border shadow-sm hover:bg-white flex items-center gap-2"><Store className="w-4 h-4"/> Магазин</button>
              <button onClick={()=>setShowSettings(true)} className="px-3 py-2 rounded-lg bg-white/80 border shadow-sm hover:bg-white flex items-center gap-2"><Settings className="w-4 h-4"/> Настройки</button>
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            <button onClick={claimDaily} disabled={dailyClaimed} className="px-3 py-2 rounded-lg bg-emerald-50 border shadow-sm hover:bg-white disabled:opacity-50">Ежедневный бонус</button>
          </div>
        </div>
        ...
        <div className="rounded-2xl bg-white/70 backdrop-blur p-4 shadow-lg border flex flex-col gap-4">
          <div>
            <h2 className="text-xl font-semibold mb-2 flex items-center gap-2"><Hammer className="w-5 h-5"/> Инструменты</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-[360px] overflow-auto pr-1">
              {toolButtons}
            </div>
          </div>
          <div>
            <h2 className="text-xl font-semibold mb-2 flex items-center gap-2"><Trophy className="w-5 h-5"/> Награды и Квесты</h2>
            <div className="space-y-2 max-h-[240px] overflow-auto pr-1">
              {ACHIEVEMENTS.map(a=>{ const got = !!achUnlocked[a.id]; const Icon = a.icon; return (
                <div key={a.id} className={`flex items-center justify-between p-2 rounded-xl border ${got? 'bg-emerald-50 border-emerald-200':'bg-white border-zinc-200'}`}>
                  <div className="flex items-center gap-2"><Icon className={`w-4 h-4 ${got? 'text-emerald-600':'text-zinc-400'}`}/><div className="text-sm">{a.title}</div></div>
        ...
        {showSettings && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 bg-black/30 backdrop-blur flex items-center justify-center p-4">
            <div className="max-w-md w-full rounded-2xl bg-white p-4 border shadow-xl">
              <div className="flex items-center justify-between mb-2"><div className="font-semibold flex items-center gap-2"><Settings className="w-4 h-4"/> Настройки</div><button onClick={()=>setShowSettings(false)} className="text-sm px-2 py-1 border rounded">Закрыть</button></div>
              <div className="space-y-3">
                <button onClick={toggleMute} className="px-3 py-2 rounded-lg bg-white/80 border shadow-sm hover:bg-white flex items-center gap-2">{muted? <VolumeX className="w-4 h-4"/> : <Volume2 className="w-4 h-4"/>}{muted? 'Звук выключен' : 'Звук включен'}</button>
